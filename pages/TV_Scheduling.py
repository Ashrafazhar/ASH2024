...
def create_schedule_table(schedule, time_slots):
    schedule_data = {"Time Slot": [f"{time_slot:02d}:00" for time_slot in time_slots], "Program": schedule}
    schedule_df = pd.DataFrame(schedule_data)
    return schedule_df

# Create the table for the final schedule
schedule_table = create_schedule_table(final_schedule, all_time_slots)

# Streamlit output
st.write("### Final Optimal Schedule Table")
